...
import numpy as np
#from TPA import data_struct as Tds
#from TPA import data_extract as Tde
...
for i, dataset in enumerate(data):
    ...
    sp.set_xlabel("IMF $B_x$")
    sp.set_ylabel("Distribution (%)")
    sp.hist(IMF[dataset]['Bx'], bins=np.linspace(-20, 20, 300), density=True, histtype="step")
    sp.hist([tpa.Bx for tpa in data[dataset] if not np.isnan(tpa.Bx)], bins=np.linspace(-20, 20, 40), label=dataset, density=True, histtype="step")
    sp.legend()
...
if savefig:
    plt.savefig("results/TPA_hist_Bx_all_{}_{}minavg_hemadjust.pdf".format(timeshift, timeshift+avgcalctime))
if plotfig:
    plt.show()
