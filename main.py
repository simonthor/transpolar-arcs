...
if __name__ == '__main__':
    ...
    for dataset, ax in zip(datasets, axes):
        ...
        if plot_type == 'hist1d':
            ...
            hist2d(dataset.tpa_values[paras_in[0]], dataset.tpa_values[paras_in[1]], dataset.total[paras_in[0]], dataset.total[paras_in[1]],
                   ax, labels=(create_label(paras_in[0]), create_label(paras_in[0])), dataset_name=dataset.name, normalize=norm)
        else:
            raise ValueError(f'Unsupported plot_type: "{plot_type}"')

    fig.tight_layout()
    if savefig:
        plt.savefig(figure_filename.format(variable, timeshift, timeshift + avgcalctime, f'_norm{ylim}' if norm else ''))
